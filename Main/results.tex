...
\begin{proposition}[Optimal Deep Orthogonal Decomposition] \label{prop: DOD Existence}
    ...
    \begin{equation*}
        \int_{\Theta \times \Theta' \times [0, T]} 
        \norm{\uht - \V_{\mu, t}^2 \V_{\mu, t}^T \G \uht} d(\mu, \nu, t)
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \begin{align}
            ...
            &\leq C_1 \sup\limits_{(\mu, \nu, t), (\mu', \nu', t') \in \Theta \times \Theta' \times [0, T]}
            \opnorm{\A_\text{P}^T \G} \, \euknorm{\uht - u_h^{\mu', \nu', t'}} \label{align: diam Vn 4} \\
        ...
        $\opnorm{\A_\text{P}^T \G} = 1$ by $\G$-orthonormality of $\A_\text{P}$, together with the
...
As a matter of fact, in the following we set $N_A = N_h$.
...
\begin{theorem} \label{theo: PAC bound for DOD-DL-ROM}
    ...
    (\ref{assumption additional lipschitz}).
    Then there is $N_\text{data} = N_\text{data}(\delta, \varepsilon)$, 
