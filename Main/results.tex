...
\begin{remark}
    Generally we can \emph{not} employ Yarotsky Theorem (\ref{theo: yarotsky}) here, since the optimal objective
    functional, as described above, can only be shown to be Lipschitz continous under our current 
    assumptions, as given above, and Yarotsky
    does require $C^2$-smoothness. However, we will set up the proof for the following lemma in such a way,
...
\begin{proof}
    ...
    \begin{equation*}
        J(\mu, t, \V) := \int_{\Theta'} \norm{\uht - \V \V^T \G \uht} d(\nu) = \int_{\Theta'} \euknorm{\uhtprered - 
        \tilde{\V} \tilde{\V}^T \uhtprered} d(\nu),
    \end{equation*}
    for $u_\text{pre-red}^{\mu, \nu, t} := \A^T \G \uht \in \R^{N_A}$ for each $\mu \in \Theta, \nu \in \Theta'$
    and $t \in [0, T]$, and let some $\varepsilon > 0$ be fixed. 
    ...
    $u := \uhtprered$ and $u' := u_\text{pre-red}^{\mu', \nu, t'}$, and the projectors $P := \tilde{\V} \tilde{\V}^T$, $P'$ 
    analogously:
    ...
    \end{align*}
    and is thus Lipschitz continous aswell.
    \notdone{big step here}
    Hence there is a Borel measurable map $s: \Theta \times [0, T] \to [0, 1]^{N_A \times N}$ 
    producing w.l.o.g. orthonormal
    matrices given by
    \begin{equation*}
        s: (\mu, t) \mapsto \underset{{\tilde{\V} \in [0, 1]^{N_A \times N}}}{\text{argmin}}  J(\mu, t, \A \tilde{\V}).
    \end{equation*}
    This map is bounded (given by continuity of J and compactness of its image) and therefore 
    ...
    \begin{equation*}
        \tilde{\V}_0 := P\left(P([\tilde{\V}_0]_{1 1}, \dots, [\tilde{\V}_0]_{1 N}), \dots, P([\tilde{\V}_0]_{N_A  1}, \dots,
...
\begin{proof}
    ...
    on the complexity is suggested by Theorem (\ref{theo: yarotsky}), since the $L^\infty$ error correlates with the 
