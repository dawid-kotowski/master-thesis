...
\begin{remark} \label{remark: discrete optimal DOD}
    ...
    \begin{multline*}
        \sup\limits_{(\mu, t) \in \Theta \times [0, T]} \frac{1}{N_{s_2}} \sum_{\nu \in \Pcal_2}
        \norm{\uht - \V_{\mu, t}
        \V_{\mu, t}^T \G \uht} \\
        < \varepsilon + \sup\limits_{(\mu, t) \in \Theta \times [0, T]} \frac{1}{N_{s_2}} \min\limits_{\V \in \R^{N_h \times N'}} 
        \sum_{\nu \in \Pcal_2} 
...
\begin{proof}
    ...
    \Theta(\Phi_{\tilde{\V}})$ for the underlying DOD neural network in accordance to Proposition (\ref{prop: DOD Existence})
    \begin{equation}
    ...
    (\ref{prop: DOD Existence}) for the lemma-specific $\varepsilon > 0$ 
    \begin{align*}
...
\begin{equation} \label{eq: definition of N, N'}
    ...
    \sum_{k > n} \sigma(\mu, t)_k^2 \leq \frac{m^2}{16} 
    \varepsilon^2 \right)
\end{equation}
...
\begin{equation*}
    \psi_N := R_\rho(\Psi_A(\theta_\text{D})) \in \R^{N},  \quad \phi_n := R_\rho(\Phi_n(\theta_\text{DF})) \in \R^N
\end{equation*}
...
\begin{equation*}
    \V_{\mu, t} := \A \cdot R_\rho(\tilde{\V}_{\mu, t}(\theta_\text{DOD})) \in \R^{N_h}, \quad 
    \phi_{N'} := R_\rho(\Phi_{N'}(\theta_{\text{DF}_2})) \in \R^{N'}
\end{equation*}
...
\begin{equation*}
    \V_{\mu, t} := \A \cdot R_\rho(\tilde{\V}_{\mu, t}(\theta_\text{DOD})) \in \R^{N_h}, \quad 
    \psi_{N'} := R_\rho(\Psi_{N'}(\theta_{\text{D}_2})) \in \R^{N'}, \quad
    \phi_{n} := R_\rho(\Phi_n(\theta_{\text{DF}_3})) \in \R^n,
\end{equation*}
...
general, the perfect embedding is implied for $N'$ aswell; let $\phi_* \in C^s(\R^n; \R^{N'})$ 
and $\phi_*' \in C^{s'}(\R^{N'}; \R^n)$ be the maps attaining the equivalent for $N'$.
...
\end{equation*} 
and $C_1', C_2'$ analogously for $\phi_*$ and $\phi_*'$. 
...
\begin{theorem} \label{theo: PAC bound for POD-DL-ROM}
    ...
    a constant $c = c(\Theta, \Theta', \Tcal, L, C_1, C_2, p, q, n, s, s')$ and 
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \in \Theta \times \Theta' \times \Tcal \}$, then the image of the optimal
        embedding $\mathcal{V}_n = \psi_*'(\mathcal{S}_N) \in \R^n$ is such that $\text{diam}(\mathcal{V}_N) 
        \leq L C_1 \text{diam}(\Theta \times \Theta' \times [0, T])$, given 
        the Assumption (\ref{assumption parameter-to-solution map}) by continuity.
        In fact, consider for $w, w' \in \mathcal{V}_n$ the diameter notation in an $W^{s, \infty}$ sense,
        \begin{align*}
            \norm{w - w'}_{W^{s, \infty}(\mathcal{V}_n)}
            &\leq \max\limits_{\euknorm{\alpha} \leq s}
            \opnorm{D^\alpha \psi_*(w) -} \\
            &= \max\limits_{\euknorm{\alpha} \leq s'}
            \opnorm{D^\alpha \psi_*'} \cdot \left( \sup\limits_{
                (\mu, \nu, t), (\mu', \nu', t') \in \Theta \times \Theta' \times [0, T]} \euknorm{
                \A^T \G (\uht - u_h^{\mu', \nu', t'})} \right)
        \end{align*}
        ...
        \begin{gather} \label{eq: decoder error for pod-dl-rom}
            ...
            \frac{\vert \, (\psi - \psi_*)(v) - (\psi - \psi_*)(v') \, \vert}{\vert \, v - v' \, \vert} 
            < \frac{m}{6} \vert \, \Theta \times \Theta' \times \Tcal \, \vert^{-1/2} \varepsilon,
        \end{gather}
        with $L_{n \to N} = c \log(\varepsilon^{-1})$ layers and $\omega_{n \to N} = c N 
        ...
        \end{equation}
        with $L_{(p+q+1) \to n} = c \log(\varepsilon^{-1})$ layers and 
        $\omega_{(p+q+1) \to n} = c n \varepsilon^{-(p+q+1)} \log(\varepsilon^{-1})$ active weights.
    \end{enumerate}
...
\begin{proof}
    ...
    of the theoretical POD matrix $V^*(\mu, t)$ by employing the Remark (\ref{remark: discrete optimal DOD}) with 
    the bound
    \begin{equation*}
