...
\begin{remark} \label{remark: discrete optimal DOD}
    ...
    \V_{\mu, t}(\theta^*_\text{DOD})$
    \begin{equation*}
        \sup\limits_{(\mu, t) \in \Theta \times [0, T]} \frac{1}{N_{s_2}} \sum_{\nu \in \Pcal_2}
        \norm{\uht - \V_{\mu, t}
        \V_{\mu, t}^T \G \uht} 
        < \varepsilon + \sup\limits_{(\mu, t) \in \Theta \times [0, T]} d_{N'}(\mathcal{S}_{\mu, t}).
    \end{equation*}
\end{remark}
...
of Remark (\ref{remark: discrete optimal DOD}). 
...
set up $N$ and $N'$ for $\Scal := \{\uht \vert (\mu, \nu, t) \in \Theta \times \Theta' \times [0, T] \}$ and
$\Scal_{\mu, t} := \{\uht \vert \nu \in \Theta'\}$ for any $(\mu, t) \in \Theta \times [0, T]$ as follows
\begin{equation}
    N = \underset{n \in \N}{\operatorname{arg \, min}} \left( d_n(\Scal) \leq \frac{m^2}{9} \varepsilon^2 \right),
    \quad N' = \underset{n \in \N}{\operatorname{arg \, min}} \left( \sup\limits_{(\mu, t)} 
    d_n(\Scal_{\mu, t}) \leq \frac{m^2}{16} 
    \varepsilon^2 \right)
\end{equation}
with $\varepsilon > 0$ yet unspecified. Note, that under Assumption (\ref{assumption KnW decay}) the hierarchy
...
\begin{equation*}
    ...
    \mapsto \A_\text{P} \cdot \left( \psi_A \circ \phi_n \right) (\mu, \nu, t),
\end{equation*}
the POD-DL-ROM under weights $\theta_\text{POD-DL-ROM} = (\theta_\text{D}, \theta_\text{DF})$ with 
\begin{equation*}
    \psi_A := R_\rho(\Psi_A(\theta_\text{D})) \in \R^{N_A},  \quad \phi_n := R_\rho(\Phi_n(\theta_\text{DF})) \in \R^N
\end{equation*}
from the Subsection (\ref{subsec: pod-dl-rom}).

Similarily, refine the approximate parameter-to-solution map for the DOD+DNN into
\begin{equation*}
...
\end{equation*}
the DOD+DFNN under weights $\theta_\text{DOD+DFNN} = (\theta_\text{DOD}, \theta_{\text{D}_2}, 
...
\begin{proof}
    ...
    stochastic in nature, and thus we will employ Remak (\ref{remark: discrete optimal DOD}) to find an auxilary term,
