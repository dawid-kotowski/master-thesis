...
\begin{definition}[Optimal DOD] \label{def: DOD functional}
    ...
    we define $J: \Theta \times [0, T] \times [0, 1]^{N_h \times N'} \to \R$ the \emph{objective DOD functional} via
    \begin{equation*}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        \norm{u_h^{\mu', \nu, t'} - \V' (\V')^T \G 
        u_h^{\mu', \nu, t'}} \, \right\vert d(\nu) \\
        &= \int_{\Xcal} \left\vert \, \left(u_h^{\mu, \nu, t} - \V 
        \V^T \G u_h^{\mu, \nu, t}\right) - 
        \left(u_h^{\mu', \nu, t'} - \V' (\V')^T \G
        u_h^{\mu', \nu, t'}\right) \, \right\vert d(\nu) \\
        &\leq \int_{\Xcal} \left\vert (\I- \V \V^T \G)(\uht - 
        u_h^{\mu', \nu, t'} )  \right\vert d(\nu) + M \vert \Xcal \vert \opnorm{\V \V^T \G - 
        \V' (\V')^T \G}   \\
        &\leq \vert \Xcal \vert (L  + 2  M ) \left( \euknorm{\mu - \mu'} + \euknorm{t - t'} + \euknorm{\V - \V'}\right),
    \end{align*}
...
\begin{proposition}[Optimal Deep Orthogonal Decomposition] \label{prop: DOD Existence}
    ...
    \begin{equation*}
        ...
        \leq \varepsilon  + c_\A' + \int_{\Theta \times [0, T]} d_{N'}(\mathcal{S}^\text{pre-red}_{\mu, t}) d(\mu, t).
    \end{equation*}
\end{proposition}
\begin{proof}
    Let $N', N_A, N_h \in \N$ be fixed by Assumption (\ref{assumption: dimension hierarchy}). 
    ...
    \begin{equation*}
        ...
        \int_{\Theta \times [0, T]} \euknorm{s(\mu, t)} \, d(\mu, t)  < \infty.
    \end{equation*}
    ...
    \begin{equation*}
        \norm{s - R_\rho(\tilde{\V}_0)}_{L^1(\Theta \times [0, T]; \R^{N_A \times N'})} \leq
        \varepsilon.
    \end{equation*}
    ...
    \begin{equation*}
        \int_{\Theta \times [0, T]} \norm{\V_{\mu, t} - \A s(\mu, t)} d(\mu, t)
        \leq \int_{\Theta \times [0, T]} \euknorm{R_\rho(\tilde{\V}_0)(\mu, t) - s(\mu, t)}
        d(\mu, t)
        \leq \varepsilon,
    \end{equation*}
    by the $1$-Lipschitz propert of $R_\rho(\zeta)$ and the same argument as was used in the proof
    ...
    \begin{multline} \label{eq: last step for optimal DOD}
        ...
        \sup\limits_{\nu \in \Theta'} \euknorm{\uhtprered - \tilde{\V} \tilde{\V}^T \uhtprered} \right) d(\mu, t).
    \end{multline}
\end{proof}
...
\begin{equation*}
    K(\mu, t) := \frac{\euknorm{\Theta'}}{N_{s_2}} U \G U^T,
\end{equation*}
that fulfill
\begin{equation*}
    \frac{\euknorm{\Theta'}}{N_{s_2}} 
        \min\limits_{\V \in \R^{N_h \times n}} 
        \sum_{\nu \in \Pcal_2} 
        \norm{\uht - \uht \V \V^T \G \uht}^2 = \sum_{k > n} \sigma(\mu, t)^2_k,
\end{equation*}
...
\begin{proof}
    ...
    (\ref{remark: discrete optimal DOD}) for some fixed $\delta > 0$ and its subsequently defined virtual discrete 
...
\begin{proof}  
    ...
    \begin{equation*}
        ...
        \frac{\norm{\uht - \V_{\mu, t} \V_{\mu, t}^T \G \uht}}{\norm{\uht}} d(\mu, \nu, t),
    \end{equation*}
    ...
    under the continuous correlation matrix, obtained for $N_{s_2} \to \infty$, 
    ...
    \begin{align}
        ...
        \int_{\Theta'} \norm{\uht - \A^\infty_{\mu, t} (\A^\infty_{\mu, t})^T \G \uht} d(\nu) d(\mu, t)
...
\begin{assumption}
    Let the objective DOD functional $J: \Theta \times [0, T] \times [0, 1]^{N_A \times N'}$ be given
    ...
    \begin{equation*}
        ...
        \underset{{\tilde{\V} \in [0, 1]^{N_A \times N'}}}{\text{argmin}}  
        J(\mu, t, \A \tilde{\V})
    \end{equation*}
    be $\hat{L}$-Lipschitz.
\end{assumption}
