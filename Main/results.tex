...
\begin{proposition} \label{prop: regularity discussion for POD vs DOD}
    ...
    \begin{equation*}
        m \gtrsim \frac{p+1}{\frac{n}{s-1} - \log(\varepsilon^{-1})^{-1} \log(N_h) + 
...
\begin{proof}
    ...
    \begin{equation*}
        ...
        = \log(\varepsilon^{-1})^{-1} \log(N_h) - \frac{\beta - \alpha}{\beta \alpha},
    \end{equation*}
...
\begin{remark}
    ...
    From the heuristic for $s \gg s' \geq 2$, and usually $p, q \approx 1$ implying
    $n \leq 2(p+q+1)+3 \approx 9 \ll s-1$, 
    we might infer, that the contribution of $n/(s-1)$
    can be neglected, thus offering $m$ to be of order $\approx 1 - 2$, assuming that the difference between
    $\alpha$ and $\beta$ is .
\end{remark}