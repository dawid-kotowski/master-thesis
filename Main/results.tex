...
$r, m$ are close to $1$, in a sense that $n/(s-1) < (p+1)/m$ and $n/(s-1) = (p+q+1)/r$. 
This directly results in 
\begin{align*}
    \omega_\text{POD-DL-ROM} &= \Ocal\left( N \varepsilon^{-n/(s-1)} \log(\varepsilon^{-1}) + 
    n \varepsilon^{-(p+q+1)/r} \log(\varepsilon^{-1})\right) \\
    &\lesssim \Ocal\left( N \varepsilon^{-n/(s-1)} \log(\varepsilon^{-1})\right) \\
    &\ll \Ocal\left( N_h N' \varepsilon^{-(p+1)/m} \log(\varepsilon^{-1}) \right) \\
...
the DOD to the complexity is minimal and we get
\begin{align*}
...
\end{align*}
This can be easily attributed to the fact, that $N' + n \lesssim N$, because of Equation (\ref{eq: order of N, N'}).
Here, we have also still prescribed the dominance of the decoder complexity over the forward pass,
therefore awarding the DOD-DL-ROM with a huge bonus due to the inherently better approximation under the smooth
