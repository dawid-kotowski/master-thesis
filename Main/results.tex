...
\begin{proposition}
    ...
    \end{equation*}
    and the optimal DOD map $s \in W^{m, \infty}(\Theta \times [0,T])$,
    such that 
    \begin{equation*}
        m \gtrsim \frac{p+1}{\frac{n}{s-1} - \log(\varepsilon^{-1}) \log\left(\frac{N - N'}{N_h N'}\right)}
    \end{equation*}
    is true, guarantees that either
    \begin{equation*}
        \omega_\text{DOD-DL-ROM} \lesssim \omega_\text{POD-DL-ROM}, \quad \text{or} \quad
        \omega_\text{DOD+DFNN} \lesssim \omega_\text{POD-DL-ROM},
    \end{equation*}
    depending on $r \geq 1$ setting the regularity 
    ...
    \begin{equation*}
        m \gtrsim \frac{p+1}{\frac{n}{s-1} + \frac{\alpha - \beta}{\alpha \beta} 
        + \log_\varepsilon\left(\frac{1}{N_h}\right)}.
    \end{equation*}
\end{proposition}
\begin{proof}
    For the following, we want to start with the wanted statement and simply rearrange the wanted inequality
    until it poses a lower bound for $m$. Therefore, we may write for the case, that if $r \geq (s-1)(p+q+1)/n$,
    we have that the number of maximal weights is larger for the DOD-DL-ROM by the previous discussion, and
    therefore 
    \begin{equation*}
        \omega_\text{DOD-DL-ROM} \lesssim \omega_\text{POD-DL-ROM}
    \end{equation*}
    is enough as a starting point. This itself implies, since we can subtract the latent dynamics contribution
    ...
    \end{equation*}
    Then taking the logarithm with regard to $\varepsilon < 1$ as base, i.e. with $\log(\varepsilon)<0$, therefore 
    ...
    \end{equation*}
    We can follow this up, with the argument, that by Assumption (\ref{assumption: dimension hierarchy}),
    we have that $0 < (N-N')/(N_hN') \leq (N + N')/(N_hN') < 1$. This means that the both sides are strictly positive,
    hence resulting in the wanted bound.
\end{proof}