...
In this final chapter, the focus will be to briefly review the results, and present potential further aspect 
of scientific research in that area. For this, we want to lastly seperated
the portion into two sections, answering both concluding questions; what was achieved in this thesis
...
Orthogal Decomposition based Deep Learning Reduced Order Model (DOD-DL-ROM)
based on the founding idea of \cite{franco2024} and a crossing with the POD-DL-ROM
given by \cite{fresca2022}, and
...
Proper Orthogonal Decomposition based one, as long as the aimed upon error tolerance $\varepsilon$ is significantly low,
...
\end{equation*}
where $s$ denotes the implied regularity of the optimal non-linear embedding function between the coefficient
...
optimal projection
derived from the paper \cite{franco2024} in a context of approximation theory lifted, in notation and
content, from \cite{gühring2019nn}. Furthermore, we could provide a distinct error decomposition upper and
lower bound for any such Deep Orthogonal Decomposition based model, which we went on to visualize
in the last of three examples, where the other two served the purpose of showing one instance of the
...
For this, we will list a few of these talking points regarding the proposed DOD-DL-ROM and any other
...
\begin{enumerate}
    ...
    \item[-] For the Upper Complexiy Bounds, we have assumed, that $N_h = N_A$ to make the discussion regarding
    ...
    \item[-] As it stands, the DOD uses $t \in [0, T]$ as a simple parameter and thus introduces this as by
    injection as a factor of slow KnW decay, which in some cases it might not be. Hence, an alternative
